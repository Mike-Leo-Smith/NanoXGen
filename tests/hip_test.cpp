...
public:
    ...
    [[nodiscard]] std::vector<T> download() const {
        ...
        check_hip(hipMemcpy(result.data(), _data, bytes(), hipMemcpyDeviceToHost),
...
void test_packed_generation(
    ...
    check_hip(hipDeviceSynchronize(), "packed generation synchronize");
    ...
    require(actual_counts == reference.point_counts, "HIP point counts must be exact");
    ...
    require_close(point_errors, "HIP packed positions");
    require_close(radius_errors, "HIP packed radii");
    ...
    for (std::size_t i = 0u; i < reference.roots.size(); ++i) {
        require(actual_roots[i].triangle_index == reference.roots[i].triangle_index,
                "HIP root triangle identity must be exact");
    ...
    require_close(root_errors, "HIP roots");
    require_close(uv_errors, "HIP root attributes");
}
...
void test_deformed_motion(
    ...
    check_hip(launch_generate_motion_hip(
                   device_asset, samples, 2u, params, output, {128u}),
               "launch_generate_motion_hip");
    check_hip(hipDeviceSynchronize(), "motion generation synchronize");
    ...
    require_close(rest_errors, "HIP rest motion sample");
    require_close(moved_errors, "HIP deformed motion sample");
}
...
int main() try {
    ...
    if (device_error != hipSuccess || device_count == 0) {
        std::cout << "SKIP: no usable HIP device ("
    ...
    std::cout << "all NanoXGen HIP parity tests passed\n";
    return 0;
} catch (const std::exception &error) {
    std::cerr << "HIP test failure: " << error.what() << '\n';
    return 1;
}
